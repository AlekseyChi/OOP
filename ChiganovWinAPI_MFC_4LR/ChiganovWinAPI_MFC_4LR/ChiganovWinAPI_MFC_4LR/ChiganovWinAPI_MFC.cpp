...
// ChiganovWinAPI_MFC.cpp: определяет поведение классов для приложения.
//
...
BEGIN_MESSAGE_MAP(CChiganovWinAPIMFCApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CChiganovWinAPIMFCApp::OnAppAbout)
...
// Создание CChiganovWinAPIMFCApp

CChiganovWinAPIMFCApp::CChiganovWinAPIMFCApp() noexcept
{

	// TODO: замените ниже строку идентификатора приложения строкой уникального идентификатора; рекомендуемый
	// формат для строки: ИмяКомпании.ИмяПродукта.СубПродукт.СведенияОВерсии
	SetAppID(_T("ChiganovWinAPIMFC.AppID.NoVersion"));

	// TODO: добавьте код создания,
	// Размещает весь важный код инициализации в InitInstance
}

// Единственный объект CChiganovWinAPIMFCApp

CChiganovWinAPIMFCApp theApp;


// Инициализация CChiganovWinAPIMFCApp

BOOL CChiganovWinAPIMFCApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CChiganovWinAPIMFCDoc),
		RUNTIME_CLASS(CMainFrame),       // основное окно рамки SDI
		RUNTIME_CLASS(CChiganovWinAPIMFCView));
	if (!pDocTemplate)
		return FALSE;
...
// Обработчики сообщений CChiganovWinAPIMFCApp
...
void CChiganovWinAPIMFCApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}
